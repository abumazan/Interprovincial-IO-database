...
%% read
load TrdShare.mat impshare2002
impshare = impshare2002;

impshare_dt = impshare(1:end,1:3);
impshare_flag = impshare(1:end,4);
impshare_dt_3d = reshape(impshare_dt,42,31,3);
impshare_flag_2d = reshape(impshare_flag,42,31);
impshare_flag_logic = zeros(42,31);
...
B_A_ini = cell(42,31); % results of b_a
GetOrNot = nan(42,31);
inve_d = cell(42,31);
inve_provin = cell(42,31);
inve_imp_add = cell(42,31);
            ...
            if GetOrNot(i,j)==0 % solution not found
                ...
                if  GetOrNot(i,j)==0 % solution not found
                    ...
                    if  GetOrNot(i,j)==0 % solution not found
                        ...
                        [b_a,~,GetOrNot(i,j)] = wRAS1(a,r_ctrl,c_ctrl,weight,1000,accu,'-deal');
                        if GetOrNot(i,j)==0 % solution not found
%                             warning('No solution when 10^(-11)')
                            accu = 10^(-10);
                            [b_a,~,GetOrNot(i,j)] = wRAS1(a,r_ctrl,c_ctrl,weight,1000,accu,'-deal');
                            if  GetOrNot(i,j)==0 % solution not found
                                accu = 10^(-9);
